...
$env_values = loadyaml('/vagrant/puphpet/local-environment.yaml')
...
if hash_key_equals($apache_values, 'install', 1) {
  ...
  if count($apache_vhosts) > 0 {
    each( $apache_vhosts ) |$key, $vhost| {
      ...
      $vhost_with_env = merge($vhost, $env_values[$key])

      create_resources(apache::vhost, { "${key}" => merge($vhost_with_env, {
